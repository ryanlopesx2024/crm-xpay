...
// ── Metadata for non-action nodes ────────────────────────────────────────────
const ACTION_META: Record<string, { icon: typeof MessageSquare; title: string; subtitle: string; iconColor: string; iconBg: string }> = {
  SEND_MESSAGE:  { icon: MessageSquare, title: 'Mensagens',       subtitle: 'Envie, receba e armazene respostas', iconColor: 'text-emerald-600', iconBg: 'bg-emerald-100' },
  ...
  HTTP_REQUEST:  { icon: Globe,         title: 'Webhook HTTP',    subtitle: 'Envia dados para uma URL externa',   iconColor: 'text-amber-600',   iconBg: 'bg-amber-100'   },
        ...
        <div className="px-3 py-3 space-y-2.5">
              ...
              <div className="flex flex-wrap gap-1">
                {VARIABLES.map((v) => (
                  <button
                    ...
                    className="px-2 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-medium rounded-lg hover:bg-blue-100 transition-colors"
                  >
                    {v.label}
                  </button>
                ))}
            ...
            <>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={item.variable || ''}
                  onChange={(e) => onChange({ ...item, variable: e.target.value })}
                  className={fieldClass}
                  placeholder="Ex: nome_resposta"
                />
              </div>
            </>
          )}
          ...
          {item.type === 'delay' && (
            <div className="flex gap-2">
              <input
                ...
                min={1}
                value={item.delay || 1}
                onChange={(e) => onChange({ ...item, delay: parseInt(e.target.value) })}
                className={`${fieldClass} w-24`}
              />
              <select
                value={item.unit || 'SECONDS'}
          ...
          {item.type === 'buttons' && (
            <div className="space-y-2">
                ...
                <div key={bi} className="flex gap-2 items-center">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      onChange({ ...item, buttons: btns });
                    }}
                    className={`${fieldClass} flex-1`}
                    placeholder={`Botão ${bi + 1}`}
                  />
                  <button
                    onClick={() => onChange({ ...item, buttons: (item.buttons || []).filter((_, idx) => idx !== bi) })}
              ...
              <button
                onClick={() => onChange({ ...item, buttons: [...(item.buttons || []), { label: '', value: String(Date.now()) }] })}
                className="w-full flex items-center justify-center gap-1.5 py-2 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl text-[11px] text-slate-500 hover:border-blue-400 hover:text-blue-500 transition-colors"
              >
...
export default function ConfigPanel({ node, onClose, onSave }: ConfigPanelProps) {
  ...
  useEffect(() => {
    ...
    const d = { ...node.data };
    setFormData(d);
    // Migrate old string `message` → contents array
    if (node.type === 'action' && d.action === 'SEND_MESSAGE') {
  ...
  const handleSave = () => {
    const finalData = node.type === 'action' && formData.action === 'SEND_MESSAGE'
      ? { ...formData, contents, message: contents.filter((c) => c.type === 'text').map((c) => c.text).join('\n') }
      : formData;
  ...
  const updateItem = (id: string, updated: ContentItem) =>
  ...
  // Resolve header meta
  const action = formData.action as string | undefined;
  const meta =
    node.type === 'action' && action && ACTION_META[action]
      ? ACTION_META[action]
  ...
  return (
    <div className="absolute right-0 top-0 h-full w-[320px] bg-white dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700 shadow-2xl z-10 flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
        <button onClick={onClose} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{meta.subtitle}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* ── SEND_MESSAGE content builder ─────────────────────────────── */}
        {node.type === 'action' && formData.action === 'SEND_MESSAGE' && (
          <>
            {/* Connection */}
            <div className="px-4 pt-4 pb-3 border-b border-slate-100 dark:border-slate-700">
              <p className={labelClass}>Conexão</p>
              <div className="flex items-center gap-2 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 bg-white dark:bg-slate-700">
                <div className="w-5 h-5 bg-green-500 rounded-full flex items-center justify-center flex-shrink-0">
                  <MessageSquare size={11} className="text-white" />
                </div>
                <input
                  type="text"
                  value={String(formData.connection || '')}
                  onChange={(e) => update('connection', e.target.value)}
                  placeholder="Ex: 3802"
                  className="flex-1 text-sm text-slate-700 dark:text-slate-200 bg-transparent focus:outline-none placeholder:text-slate-300"
                />
              </div>
              <p className="text-[9px] text-slate-400 mt-1.5">Deixe em branco para usar a conexão dos blocos anteriores.</p>
            </div>
            ...
            <div className="px-4 py-4 space-y-2.5">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-xl text-xs text-slate-500 dark:text-slate-400 hover:border-blue-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors"
              >
                <Plus size={14} />
                Adicionar elemento
              </button>
        ...
        {node.type === 'action' && formData.action !== 'SEND_MESSAGE' && (
          <div className="px-4 py-4 space-y-4">
            <div>
              ...
              <select value={String(formData.action || '')} onChange={(e) => update('action', e.target.value)} className={fieldClass}>
            ...
            {(formData.action === 'ADD_TAG' || formData.action === 'REMOVE_TAG') && (
              <div>
                <p className={labelClass}>Nome da tag</p>
                <input type="text" value={String(formData.tag || '')} onChange={(e) => update('tag', e.target.value)} className={fieldClass} placeholder="Ex: cliente-vip" />
              </div>
            )}
            {formData.action === 'HTTP_REQUEST' && (
              <>
                <div>
            ...
            {formData.action === 'ASSIGN_AGENT' && (
              <div>
                <p className={labelClass}>Atendente</p>
                <input type="text" value={String(formData.agent || '')} onChange={(e) => update('agent', e.target.value)} className={fieldClass} placeholder="Nome ou ID do atendente" />
              </div>
            )}
          </div>
        )}
        ...
        {node.type === 'delay' && (
          <div className="px-4 py-4 space-y-4">
            <p className="text-xs text-slate-500 dark:text-slate-400">Aguarda um tempo antes de continuar para o próximo bloco.</p>
